    ...
    queueLimit: 0 // Número ilimitado de solicitudes en espera
    });

    // Verificar la conexión (solo para asegurar que el pool está funcionando correctamente)
    ...
    try {
        ...
        pool.query(query, [username, hashedPassword], (error, result) => {
        if (error) {
    ...
    pool.query('DELETE FROM empleados WHERE id = ?', [id], (error, result) => {
        if (error) {
        return handleDbError(error, res);
        }
        if (result.affectedRows === 0) {
